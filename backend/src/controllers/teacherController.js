...
const destroyFromCloudinary = async (url, resourceType = "raw") => {
  try {
    if (!url || !url.includes("cloudinary.com")) return;
    // Extract public_id from URL
    // e.g. https://res.cloudinary.com/cloud/raw/upload/v123/lms/inst123/pdfs/pdf_1234.pdf
    const parts = url.split("/upload/");
    if (parts.length < 2) return;
    // Remove version segment (v123/) and file extension
...
export const getTeacherDashboard = async (req, res) => {
  try {
    ...
    const courseWhere = { instituteId, teacherId };
    ...
    const [
      ...
      prisma.courseModule.count({ where: { course: courseWhere } }),
...
export const getTeacherCourses = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "Failed to fetch teacher courses" });
  }
};
...
export const getTeacherCourseById = async (req, res) => {
  try {
    ...
    const course = await prisma.course.findFirst({
      where: { id: courseId, instituteId, teacherId },
      include: {
        modules: {
          orderBy: { order: "asc" },
...
export const createModule = async (req, res) => {
  try {
    ...
    const course = await prisma.course.findFirst({
      where: { id: courseId, instituteId, teacherId },
      include: { modules: { orderBy: { order: "desc" }, take: 1 } },
    });
    ...
    const moduleData = await prisma.courseModule.create({
      data: { title: title.trim(), order: nextOrder, courseId },
    });
...
export const updateModule = async (req, res) => {
  try {
    ...
    const existingModule = await prisma.courseModule.findFirst({
      where: { id: moduleId, course: { instituteId, teacherId } },
    });
    ...
    const updatedModule = await prisma.courseModule.update({
      where: { id: moduleId },
      data: {
        title: typeof title === "string" && title.trim() ? title.trim() : existingModule.title,
...
export const deleteModule = async (req, res) => {
  try {
    ...
    // Delete all Cloudinary files from lessons in this module
    for (const lesson of existingModule.lessons) {
      if (lesson.videoUrl) await destroyFromCloudinary(lesson.videoUrl, "video");
      if (lesson.pdfUrl) await destroyFromCloudinary(lesson.pdfUrl, "raw");
    }
...
export const createLesson = async (req, res) => {
  try {
    ...
    if (!type || !["VIDEO", "PDF", "TEXT"].includes(type)) {
      return res.status(400).json({ success: false, message: "Valid lesson type is required" });
    }

    if (type === "TEXT" && !content?.trim()) {
      return res.status(400).json({ success: false, message: "Text content is required for TEXT lessons" });
    }
...
export const updateLesson = async (req, res) => {
  try {
    ...
    const existingLesson = await prisma.lesson.findFirst({
      where: { id: lessonId, module: { course: { instituteId, teacherId } } },
    });

    if (!existingLesson) {
      return res.status(404).json({ success: false, message: "Lesson not found or access denied" });
    }

    const nextType = type || existingLesson.type;

    // If replacing a file, delete old one from Cloudinary
    if (nextType === "VIDEO" && videoUrl && videoUrl !== existingLesson.videoUrl && existingLesson.videoUrl?.includes("cloudinary.com")) {
      await destroyFromCloudinary(existingLesson.videoUrl, "video");
    }
    if (nextType === "PDF" && pdfUrl && pdfUrl !== existingLesson.pdfUrl && existingLesson.pdfUrl?.includes("cloudinary.com")) {
      await destroyFromCloudinary(existingLesson.pdfUrl, "raw");
    }

    const updatedLesson = await prisma.lesson.update({
      where: { id: lessonId },
      data: {
        title: typeof title === "string" && title.trim() ? title.trim() : existingLesson.title,
        type: nextType,
        content: nextType === "TEXT" ? (content !== undefined ? String(content).trim() : existingLesson.content) : null,
        videoUrl: nextType === "VIDEO" ? (videoUrl !== undefined ? String(videoUrl).trim() : existingLesson.videoUrl) : null,
        pdfUrl: nextType === "PDF" ? (pdfUrl !== undefined ? String(pdfUrl).trim() : existingLesson.pdfUrl) : null,
...
export const deleteLesson = async (req, res) => {
  try {
    ...
    // Delete files from Cloudinary when lesson is deleted
    if (existingLesson.videoUrl) await destroyFromCloudinary(existingLesson.videoUrl, "video");
    if (existingLesson.pdfUrl) await destroyFromCloudinary(existingLesson.pdfUrl, "raw");
...
// ── UPLOAD PDF (simplified - uses CloudinaryStorage directly) ───────────────────
export const uploadPdf = async (req, res) => {
  try {
    ...
    // req.file.path = Cloudinary public URL (image resource type, format pdf)
    return res.status(200).json({
      success: true,
...
// ── UPLOAD VIDEO (uses multer-storage-cloudinary) ────────────────────
...
export const getTeacherCourseStudentsProgress = async (req, res) => {
  try {
    ...
    const progressRecords =
      studentIds.length > 0 && lessonIds.length > 0
        ? await prisma.lessonProgress.findMany({
            ...
            select: { studentId: true, lessonId: true, completed: true, completedAt: true },
          })
        : [];

    const progressMap = new Map();
    for (const record of progressRecords) {
      progressMap.set(record.studentId, (progressMap.get(record.studentId) || 0) + 1);
    }
    ...
    const averageCompletion =
      students.length > 0
        ? Math.round(students.reduce((s, st) => s + st.completionPercentage, 0) / students.length)
        : 0;

    const completedStudents = students.filter((s) => s.status === "COMPLETED").length;
    const inProgressStudents = students.filter((s) => s.status === "IN_PROGRESS").length;
    const notStartedStudents = students.filter((s) => s.status === "NOT_STARTED").length;
